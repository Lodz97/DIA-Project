...
if __name__ == "__main__":
    ...
    opt = np.max(np.array(pricing_arms)*np.array(arms_user_prob))
    print(opt)
    print(np.argmax(np.array(pricing_arms) * np.array(arms_user_prob)))

    for n in range(0, N_OF_EXPERIMENTS):
        print("EXP")
        print(n)
        pricing_learner = PricingTSLearner(n_arms=len(pricing_arms), arms=pricing_arms)
        ...
        for pulled_arm in range(0, len(pricing_arms)):
            collected_reward_per_arm = []
            for day in range(0, T_HORIZON):
                ...
                while daily_number_click != 0:  # the user of the day are not terminated
                    ...
                    pricing_learner.update(pulled_arm, reward)

                value_click = pricing_learner.means[pulled_arm]*pricing_arms[pulled_arm]
                comb_learner.sc_value_per_click = [value_click for i in range(0,3)]
                ...
                collected_reward_per_arm.append(sum([rewards[i] * value_click for i in range(0, len(rewards))]))
            collected_reward_adv.append(collected_reward_per_arm)
    ...
    print(optimum)
    print(np.mean(combinatorial_reward_experiment, axis=0)[2])
    plot.plot_regret_per_arm(optimum, combinatorial_reward_experiment, pricing_arms)
